...
class FeatFuse(nn.Module):
    ...
    def forward(self, x, feat):
        ...
        att = self.conv_du(feat).squeeze(-1).permute(0,2,1)
        return x * att
...
class BasicLayer_fuse(nn.Module):
    """A basic Swin Transformer layer for one stage.
    ...
    def forward(self, x, x_size, feat):
        """Forward function.
        ...
        for i, blk in enumerate(self.blocks):
            if i % 2 == 0:
                ...
                if self.use_checkpoint:
                    x = checkpoint.checkpoint(blk, x, feat)
                else:
                    x = blk(x, feat)
        # for blk in self.blocks:
        #     if self.use_checkpoint:
        #         x = checkpoint.checkpoint(blk[0], x, x_size)
        #         x = checkpoint.checkpoint(blk[1], x, feat)
        #     else:
        #         x = blk[0](x, x_size)
        #         x = blk[1](x, feat)
        if self.downsample is not None:
        ...
        return self.patch_embed(
            self.conv(
                self.patch_unembed(self.residual_group(x, x_size),
                                   x_size))) + x
class RSTB_fuse(nn.Module):
    """Residual Swin Transformer Block (RSTB) with the degradation 
       information fused.
    ...
    def __init__(self,
        ...
        super(RSTB_fuse, self).__init__()

        self.dim = dim
        self.input_resolution = input_resolution
        self.fuse_basic = fuse_basic 
        if fuse_basic == True:
            self.residual_group = BasicLayer_fuse(
                dim=dim,
                input_resolution=input_resolution,
                depth=depth,
                num_heads=num_heads,
    ...
    def forward(self, x, x_size, feat):
        ...
        Returns:
            Tensor: Forward results.
        """
        if self.fuse_basic == True:
            return self.patch_embed(
                self.conv(
                    self.patch_unembed(self.residual_group(x, x_size, feat),
                                       x_size))) + x
        else:
            return self.patch_embed(
                self.conv(
