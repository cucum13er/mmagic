...
from mmedit.models.backbones.sr_backbones.swinir.swinir_modules import PatchEmbed, PatchUnEmbed, Upsample, UpsampleOneStep
from mmedit.models.backbones.sr_backbones.swinir.swinir_rstb import RSTB, RSTB_fuse, FeatFuse
from mmedit.utils import get_root_logger
from mmcv.runner import load_checkpoint

@BACKBONES.register_module()
class SwinIRNet_HA(BaseModule):
    r""" SwinIR
    ...
    def __init__(self,
                 ...
                 fuse_basic = False,
                 **kwargs):
        super(SwinIRNet_HA, self).__init__()
        num_in_ch = in_chans
        num_out_ch = in_chans
        num_feat = 64
        ...
        for i_layer in range(self.num_layers):
            layer = RSTB_fuse(
                ...
                fuse_basic = fuse_basic,
                )
            self.layers.append(layer)
            if fuse_rstb == True:
                layer = FeatFuse(embed_dim, embed_dim)
                self.layers.append(layer)
    ...
    def forward_features(self, x, feat):
        ...
        x = self.pos_drop(x)
        if len(self.layers) < 2 * self.num_layers:
            for layer in self.layers:                
                x = layer(x, x_size, feat)
                # breakpoint()
        else:
    ...
    def forward(self, x, feat):
        ...
        if self.upsampler == 'pixelshuffle':
            # for classical SR
            x = self.conv_first(x)
            x = self.conv_after_body(self.forward_features(x,feat)) + x
            ...
            res = self.conv_after_body(
                self.forward_features(x_first, feat)) + x_first
