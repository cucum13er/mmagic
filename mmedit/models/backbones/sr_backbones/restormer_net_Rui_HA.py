...
#########################################################################################
#### added by Rui #############
class Upsample_Rui(nn.Sequential):
    """Upsample module.
    ...
    def __init__(self, scale, num_feat):
        ...
        super(Upsample_Rui, self).__init__(*m)
        
class FeatFuse(nn.Module):
    def __init__(self, channels_in, channels_out, reduction=4, deg_fsize=512):
    ...
    def forward(self, x, feat):
        ...
        feat = self.adaptive_channels(feat)
        att = self.conv_du(feat)
        return x * att                
#########################################################################################
...
# class Attention_QKV(BaseModule):
#     """Multi-DConv Head Transposed Self-Attention (MDTA)
...
#     def __init__(self, dim, num_heads, bias, contrast_feat_dim=512):
#         super(Attention_QKV, self).__init__()
#         self.num_heads = num_heads
#         self.temperature = nn.Parameter(torch.ones(num_heads, 1, 1))
#         self.q = nn.
...
# class TransformerBlock_QKV(BaseModule):
#     """Transformer Block.
...
#         self.norm1 = LayerNorm(dim, LayerNorm_type)
#         self.attn = Attention_HA(dim, num_heads, bias)
#         self.norm2 = LayerNorm(dim, LayerNorm_type)
#         self.ffn = FeedForward(dim, ffn_expansion_factor, bias)
...
#             Tensor: Forward results.
#         """
#         x = x + self.attn(self.norm1(x), feat)
#         x = x + self.ffn(self.norm2(x))

#         return x    
    
# class TransformerBlock_CA(BaseModule):
#     """Transformer Block.
...
class BasicLayer_CA(BaseModule):
    ...
    def __init__(self, dim, num_heads, ffn_expansion_factor, bias,
                 LayerNorm_type, num_blocks):
        super(BasicLayer_CA, self).__init__()
        # build blocks
    ...
    def forward(self, x, feat):
        ...
        Args:
            x (Tensor): Input tensor with shape (B, C, H, W).
            feat: Input tensor with shape (B, 512, 1, 1)
...
class Restormer_HA(BaseModule):
    ...
    def __init__(self,
                 ...
                 dual_keys=['imgL', 'imgR'],
                 fuse_type='CA', #'CA' or 'QKV'
                ):

        super(Restormer_HA, self).__init__()
        if fuse_type is 'CA':
            self.patch_embed = OverlapPatchEmbed(inp_channels, dim)

            self.encoder_level1 = BasicLayer_CA(
                                        dim=dim,
                                        num_heads=heads[0],
                                        ffn_expansion_factor=ffn_expansion_factor,
                                        bias=bias,
            ...
            self.encoder_level2 = BasicLayer_CA(
                                        dim=int(dim * 2**1),
                                        num_heads=heads[1],
                                        ffn_expansion_factor=ffn_expansion_factor,
                                        bias=bias,
            ...
            self.encoder_level3 = BasicLayer_CA(
                            dim=int(dim * 2**2),
                            num_heads=heads[2],
                            ffn_expansion_factor=ffn_expansion_factor,
                            bias=bias,
            ...
            self.decoder_level3 = BasicLayer_CA(
                            dim=int(dim * 2**2),
                            num_heads=heads[2],
                            ffn_expansion_factor=ffn_expansion_factor,
                            bias=bias,
            ...
            self.decoder_level2 = BasicLayer_CA(
                            dim=int(dim * 2**1),
                            num_heads=heads[1],
                            ffn_expansion_factor=ffn_expansion_factor,
                            bias=bias,
            ...
            self.decoder_level1 = BasicLayer_CA(
                                        dim=int(dim * 2**1),
                                        num_heads=heads[0],
                                        ffn_expansion_factor=ffn_expansion_factor,
                                        bias=bias,
            ...
            # for classical SR    
            self.scale = scale
            self.upsample = Upsample_Rui(scale=self.scale, num_feat=dim * 2**1)
            self.output = nn.Conv2d(
                                int(dim * 2**1),
                                out_channels,
                                kernel_size=3,
                                ...
                                bias=bias)
        '''        
        elif fuse_type is 'QKV':
            self.patch_embed = OverlapPatchEmbed(inp_channels, dim)

            self.encoder_level1 = nn.Sequential(*[
                TransformerBlock_QKV(
                    dim=dim,
                    num_heads=heads[0],
                    ffn_expansion_factor=ffn_expansion_factor,
                    bias=bias,
    ...
    #########################################################################################
    def forward(self, inp_img, contrastive_feat):
        """Forward function.

        Args:
            inp_img (Tensor): Input tensor with shape (B, C, H, W).
            contrastive_feat (List): Input hardware feature [(B, 512, 1, 1)]
        Returns:
            Tensor: Forward results.
        """
        # breakpoint()
        contrastive_feat = contrastive_feat[0]
        ...
        # breakpoint()
        out_enc_level1 = self.encoder_level1(inp_enc_level1, contrastive_feat)
        # breakpoint()
        inp_enc_level2 = self.down1_2(out_enc_level1)
        out_enc_level2 = self.encoder_level2(inp_enc_level2, contrastive_feat)

        inp_enc_level3 = self.down2_3(out_enc_level2)
        # breakpoint()
        out_enc_level3 = self.encoder_level3(inp_enc_level3, contrastive_feat)

        inp_enc_level4 = self.down3_4(out_enc_level3)
        latent = self.latent(inp_enc_level4, contrastive_feat)

        inp_dec_level3 = self.up4_3(latent)
        inp_dec_level3 = torch.cat([inp_dec_level3, out_enc_level3], 1)
        inp_dec_level3 = self.reduce_chan_level3(inp_dec_level3)
        out_dec_level3 = self.decoder_level3(inp_dec_level3, contrastive_feat)

        inp_dec_level2 = self.up3_2(out_dec_level3)
        inp_dec_level2 = torch.cat([inp_dec_level2, out_enc_level2], 1)
        inp_dec_level2 = self.reduce_chan_level2(inp_dec_level2)
        out_dec_level2 = self.decoder_level2(inp_dec_level2, contrastive_feat)

        inp_dec_level1 = self.up2_1(out_dec_level2)
        inp_dec_level1 = torch.cat([inp_dec_level1, out_enc_level1], 1)
        out_dec_level1 = self.decoder_level1(inp_dec_level1, contrastive_feat)
        # breakpoint()
        out_dec_level1 = self.refinement(out_dec_level1, contrastive_feat)
        ...
        if self.dual_pixel_task:
            ...
            out_dec_level1 = out_dec_level1 + self.skip_conv(inp_enc_level1)
            # breakpoint()
            out_dec_level1 = self.upsample(out_dec_level1)
            # breapoint()
            out_dec_level1 = self.output(out_dec_level1)
        # breakpoint()
        return out_dec_level1[:, :, :h*self.scale, :w*self.scale]
