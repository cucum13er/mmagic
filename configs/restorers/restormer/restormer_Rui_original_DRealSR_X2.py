# model settings
exp_name = 'restormer_pretrain_DRealSR'
scale = 2
img_size = 48
# model settings
model = dict(
        ...
        contrastive_part = dict(
                    ...
                    backbone=dict(
                        type='EasyRes',
                        in_channels=3,
                        # pretrained = '/work/pi_xiandu_umass_edu/ruima/pretrain/selfsup/moco/moco_easyres_DRealSR/weights_2000.pth',
...
train_pipeline = [
    ...
    dict(type='PairedRandomCrop', gt_patch_size=img_size*scale), # only crop lq and gt
    ...
    dict(type='PairedRandomCrop', gt_patch_size=img_size*scale), # crop the new lq and gt
...
test_pipeline = [
    ...
    dict(
        ...
        channel_order='rgb'),
    dict(type='RescaleToZeroOne', keys=['lq', 'gt']),
    dict(type='PairedRandomCrop', gt_patch_size=2000), # crop the new lq and gt
...
optimizers = dict(
                 ...
                 contrastive_part=dict(type='Adam', lr=0, betas=(0.9, 0.999) ),
                 generator=dict(type='Adam', lr=1e-4, betas=(0.9, 0.999)),
                 # generator=dict(type='SGD', lr=1e-3, momentum=0.9),
...
evaluation = dict(interval=4000, save_image=True, gpu_collect=True)
log_config = dict(
...
# load_from = '/work/pi_xiandu_umass_edu/ruima/git/mmediting_Rui_git/work_dirs/restorers/restormer/X4/pretrain_DIV2K/iter_180_beforeCrash.pth'
load_from = '/work/pi_xiandu_umass_edu/ruima/pretrain/Restormer/restormer_Rui_pretrainFromOfficialColorBlind.pth'
resume_from = None #'/work/pi_xiandu_umass_edu/ruima/git/mmediting_Rui_git/work_dirs/restorers/restormer/X4/pretrain_DIV2K/latest.pth'
workflow = [('train', 1)]